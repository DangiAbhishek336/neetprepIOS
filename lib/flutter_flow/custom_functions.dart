...
List<dynamic> getupdatedStarmarkRemove(
  List<dynamic> statusList,
  dynamic jsonValue,
  int position,
) {
  for (int i = 0; i < statusList.length; i++) {
    if (i == position) {
      statusList[i]['starmarkQuestion'] = null;
      break;
    }
  }
  return statusList;
...
List<dynamic> getUpdatedBookmarkRemoveForFlashCard(
    List<dynamic> statusList,
    dynamic jsonValue,
    int position,
    ) {
...
List<dynamic> getupdatedStarmark(
  List<dynamic> statusList,
  dynamic jsonValue,
  int position,
) {
  for (int i = 0; i < statusList.length; i++) {
    if (i == position) {
      statusList[i]['starmarkQuestion'] = jsonValue;
      break;
    }
  }
  return statusList;
...
List<dynamic> getUpdatedTimer(
  ...
  for (int i = 0; i < statusList.length; i++) {
    if (i == position) {
      statusList[i]['tempDurationInSec'] = jsonValue;
      break;
    }
  }
  return statusList;
...
List<dynamic> getUpdatedBookmarkForFlashcard(
    List<dynamic> statusList,
    dynamic jsonValue,
    int position,
    ) {
  for (int i = 0; i < statusList.length; i++) {
    if (i == position) {
      statusList[i]['bookmark'] = jsonValue;
      break;
    }
  }
  return statusList;
...
String? getBase64OfCourseId(String courseIdInt) {
  String userIdStr = "courseId:" + courseIdInt.toString();
  Codec<String, String> stringToBase64Url = utf8.fuse(base64Url);
  String encoded = stringToBase64Url.encode(userIdStr);
  /// print(encoded);
  return encoded;
}
String? getBase64OfTestId(String testIdInt) {
  String userIdStr = "courseId:" + testIdInt.toString();
  Codec<String, String> stringToBase64Url = utf8.fuse(base64Url);
  String encoded = stringToBase64Url.encode(userIdStr);
  /// print(encoded);
  return encoded;
...
List<dynamic> getUpdatedQuestionsStatusList(
  ...
  durationInSec!=null?oldAnswerList[intdexNumber]['userAnswer'] = {"userAnswer": answer, "durationInSec":durationInSec}:oldAnswerList[intdexNumber]['userAnswer'] = {"userAnswer": answer};
...
List<int> removeValueInListInSpecificPosition(
    List<int> queAnsList,
    int position,
    int element,
    ) {
  // queAnsList.insert(position, element);
  queAnsList[position] = 100;
...
List<int> removeSecondsInList(
    List<int> queAnsList,
    int position,
    int element,
    ) {
  queAnsList[position] = 13000;
  /// print(queAnsList);
  return queAnsList;
}
...
int getElapsedTime(int startTime, int currTimeMin, int currTimeSec) {
    int totalTimeSec  = currTimeMin*60+currTimeSec ;
    int startTimeInSec = startTime*60;
    int elapsedTime = startTimeInSec-totalTimeSec;
    print("totalTimeSec"+totalTimeSec.toString());
    print("startTimeInSec"+startTimeInSec.toString());
...
int convertSecondsToMinutes(int seconds) {
  final int minutes = seconds ~/ 60; // Get the whole minutes
  return minutes;
}
int convertSecondsToSecondsPart(int seconds) {
  print(seconds.toString());
  final int secondsPart = (seconds % 60); // Get the remaining seconds
  return secondsPart;
}
